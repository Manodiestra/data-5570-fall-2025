...
const API_BASE = 'http://3.85.53.16:8000';
...
export const generateRandomListing = createAsyncThunk<
  CreateItemPayload,
  void,
  { rejectValue: string; state: RootState }
>(
  'listItems/generateRandomListing',
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to generate listing');
    }
  }
);
...
const listItemsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(generateRandomListing.pending, (state) => {
        state.generateLoading = true;
        state.error = null;
      })
      .addCase(generateRandomListing.fulfilled, (state, action) => {
        state.generateLoading = false;
        state.error = null;
      })
      .addCase(generateRandomListing.rejected, (state, action) => {
        state.generateLoading = false;
        state.error = action.payload || 'Failed to generate listing';
      });
  },
});
